...
export const createFormValidator = () => [
        ...
        body('formType').notEmpty().isString().isIn(['individualAccelerated', 'individualLegacy', 'smeAccelerated', 'smeLegacy']).withMessage('formType does contain a valid value'),
        body('formTypeDesc').optional().isString().withMessage('formTypeDesc must be  a string'),
        body('formStatus').notEmpty().isString().isIn(['draft', 'published', 'deactivated',]).withMessage('formStatus does contain a valid value'),
        ...
        check('builtFormMetadata.pages').notEmpty().withMessage('Page is required').isArray({min: 1}).withMessage('Page must be an array'),
        check('builtFormMetadata.pages.*.pageProperties').isArray().withMessage('Page Properties must be an array').not().isEmpty().withMessage('Page Properties can not be empty').bail(),
        check('builtFormMetadata.pages.*.id').notEmpty().withMessage('Page id can not be empty').bail(),
        check('builtFormMetadata.pages.*.formControlType').notEmpty().withMessage('Page formControlType can not be empty').bail(),
